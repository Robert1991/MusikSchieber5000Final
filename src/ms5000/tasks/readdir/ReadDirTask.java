...
import ms5000.web.privateDump.RDFMapping;
import ms5000.web.privateDump.TagToModelService;
...
public class ReadDirTask extends Task<ObservableList<MusicTag>> {
				...
				if (isCancelled()) {
					...
					try {
						...
						if (importMode == ImportMode.CLEAR || importMode == null) {
							...
							// Adding the generated tag
							if (PropertiesUtils.getProfile().isLinkedBrainzSupport() && newFile.getIdMetaData() != null) {						
								if (LinkedBrainzUtil.checkConnection() && !newFile.getIdMetaData().isEmpty()) {
									newFile.setGeneratedTag(getGeneratedTagFromLB(newFile.getIdMetaData()));
									// Getting the tag from the private dump
									newFile.getGeneratedTag().addWholeTags(getGeneratedTagFromMSWS(newFile.getIdMetaData()));
																		
									if (PropertiesUtils.getProfile().isSmartMode()) {
										MusicTag newTag = TagUtils.buildSmartTag(newFile);
										TagUtils.determineTagState(newTag);
										newFile.setTag(newTag);
									}
								}
							}
							...
							if (!duplicateInList) {
								...
								// Adding the generated tag
								if (PropertiesUtils.getProfile().isLinkedBrainzSupport()
										&& newEntry.getMusicFile().getIdMetaData() != null) {
									if (LinkedBrainzUtil.checkConnection() && !newEntry.getMusicFile().getIdMetaData().isEmpty()) {
										
										newEntry.getMusicFile().setGeneratedTag(
												getGeneratedTagFromLB(newEntry.getMusicFile().getIdMetaData()));
										newEntry.getMusicFile().getGeneratedTag().addWholeTags(getGeneratedTagFromMSWS(newEntry.getMusicFile().getIdMetaData()));
										
										if (PropertiesUtils.getProfile().isSmartMode()) {
											newEntry = TagUtils.buildSmartTag(newEntry.getMusicFile());
											TagUtils.determineTagState(newEntry);
										}
									}
								}
	 ...
	 * 
	 */
	private GeneratedTag getGeneratedTagFromLB(MetaDataIDs ids) {
		return LinkedBrainzUtil.getTag(ids);
	}
	
	private GeneratedTag getGeneratedTagFromMSWS(MetaDataIDs ids) {
		Model tagModel = ModelFactory.createDefaultModel();
		if (ids.getRecordings().size() > 0) {
			tagModel = TagToModelService.getModelFromRecId(ids.getRecordings().get(0));
		    
		    if (!tagModel.isEmpty()) {
		    	GeneratedTag tag = RDFMapping.buildMusicTagfromRDF(tagModel);
		    	return tag;
		    } else {
		    	return null;
		    }
