...
public class ImportFilesTask extends Task<Void> {
	...
	protected Void call(){
		...
		if (tableFiles.size() > 0) {
			...
			// Uploading the meta data to the private musikschieber triple store
			if (PropertiesUtils.getProfile().isLinkedBrainzSupport()) {
				for (MusicFile musicFile : tableFiles) {
					for (String recId : musicFile.getIdMetaData().getRecordings()) {
						TagToModelService.deleteModelFromRecId(recId);
					}
					Model m =  RDFMapping.rdfBuilder(musicFile.getTag());
	...
	private void moveToTmpAndTag() {
		...
		for (MusicFile file : tableFiles) {
			...
			if (!PropertiesUtils.getProfile().isFullAutomatic()) {
				Main_Frame.getBorderPane_Center().getCentertable().getItems().remove(0);
			} else {
				Main_Frame.getBorderPane_Center().getCentertable().getItems().remove(file.getTag());
			}
	...
	private void readFilesFromTable() {
		...
		for(MusicTag tag : table.getItems()) {
			if (!PropertiesUtils.getProfile().isFullAutomatic()) {
				...
				if (tag.getStatus() != TagState.MISSINGCRITICAL && tag.getStatus() != TagState.DUPLICATE) {
					tableFiles.add(tag.getMusicFile());
					originalFiles.add(tag.getMusicFile().getFile());
				}
			}
